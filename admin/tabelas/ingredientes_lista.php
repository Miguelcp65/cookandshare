...
<head>
    ...
    <script type="text/javascript" charset="utf8" src="../../assets/js/ingredientes_lista.js"></script>
...
<?php
    ...
    if (isset($_GET["report"])){
        if ($_GET["report"] == "ingrediente-adicionado") {
          echo' <script>toastr.success("Ingrediente Adicionado com Sucesso","Sucesso!");</script>';
          echo("<script>history.replaceState({},'','$url');</script>");
       }else if ($_GET["report"] == "ingrediente-adicionado-sem-imagem") {
        echo' <script>toastr.success("Ingrediente Adicionado com Sucesso","Sucesso!");</script>';
        echo' <script>toastr.warning("Não atribuiu nenhuma imagem ao ingrediente","Atenção!");</script>';
        echo("<script>history.replaceState({},'','$url');</script>");
       }else if ($_GET["report"] == "ingrediente-editado") {
        echo' <script>toastr.success("Ingrediente Editado com Sucesso","Sucesso!");</script>';
        echo("<script>history.replaceState({},'','$url');</script>");
       }else if ($_GET["report"] == "ingredientes-eliminados") {
        echo' <script>toastr.success("Vários Ingredientes foram eliminados!","Sucesso!")</script>';
        echo("<script>history.replaceState({},'','$url');</script>");
       }else if ($_GET["report"] == "ingrediente-eliminado") {
        echo' <script>toastr.success("O Ingrediente foi eliminado com sucesso!","Sucesso!")</script>';
        echo("<script>history.replaceState({},'','$url');</script>");
       }
      }
...
<br>
    <div class="main-card mb-3 card">
        <div class="card-body">
            <div class="tabela">
                <h2 class="m-0 text-dark">Tabela de Ingredientes</h2>
            </div>
                            ...
                            echo "<tr class='trv'>
                                ...
                                <td class='tdnome'>$nome</td> ";
                                 if ($imagem != 'Sem Imagem') { 
                                  echo"
                                <td class='tdimagem'><img src='../img/ingredientes/$imagem' class='ingrei' alt='Imagem'/></td> ";
                                } else {
                                    echo" 
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input name="new_imagem" class="imagem" type="file" />
            ...
            <div id="deletemodal" class="modal fade">
                <div class="modal-dialog modal-confirm">
                    <form autocomplete="off" method="post" id="formeliminados">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h4 class="modal-title">Tem a certeza que pretende eliminar <strong class="titulodelete"></strong> ingrediente(s)?</h4>
    ...
    <?php
            if (isset($_POST['add'])) {
              $nome = $_POST['add_nome'];
              $valnome = mysqli_query($conn, "SELECT nome FROM ingredientes WHERE nome = '$nome'");
              if(mysqli_num_rows($valnome)) {
                    echo' <script>toastr.error("Já existe um ingrediente com o mesmo nome!","Erro!");</script>';  
                    echo "<script>
              ...
              if ($conn->query($sql) === true) {
                $id_img = mysqli_query($conn, "SELECT id_ingrediente FROM ingredientes WHERE nome ='$nome'");
                foreach($id_img as $rows) {
                   $img_id = $rows['id_ingrediente']; 
                    } 
                 if (!empty($_FILES['new_imagem']['name'])) {    
                    ...
                    $sql2 = "UPDATE ingredientes SET imagem='$new_imagem' WHERE id_ingrediente='$img_id'";
                    if ($conn->query($sql2) === true) {
                        echo '<script>window.location.href = "ingredientes_lista.php?report=ingrediente-adicionado";</script>';
                    }  
                }else{
                    echo '<script>window.location.href = "ingredientes_lista.php?report=ingrediente-adicionado-sem-imagem";</script>';
                }
                }
            }
        }
 ...
 if (isset($_POST['delete'])) {
    ...
    for ($i = 0; $i < count($registo_eliminar_id); $i++) {
        ...
        $img_delete = mysqli_query($conn, "SELECT imagem FROM ingredientes WHERE id_ingrediente = '$delete_id'");
                foreach($img_delete as $row) {
                   $img_del = $row['imagem'];  
                }
                if (!($img_del == 'Sem Imagem')){
                    unlink("../img/ingredientes/$img_del");
                   }
                $sql3 = "DELETE FROM ingredientes WHERE id_ingrediente = $delete_id";  
                if ($conn->query($sql2) === true) {
                }
                if (1 < count($registo_eliminar_id)){
                    echo '<script>window.location.href = "ingredientes_lista.php?report=ingredientes-eliminados";</script>';
                }else{
                    echo '<script>window.location.href = "ingredientes_lista.php?report=ingrediente-eliminado";</script>';
                }
    }
...
if (isset($_POST['update'])) {
    ...
    if ($old_nome != $update_nome){
        $select = mysqli_query($conn, "SELECT nome FROM ingredientes WHERE nome = '$update_nome'");
        if(mysqli_num_rows($select)) {
            ...
            if (!empty($_FILES['update_imagem']['name'])) {             
                $sql = "UPDATE ingredientes SET nome='$update_nome', imagem='$update_imagem' WHERE id_ingrediente=$update_id ";
                if ($conn->query($sql) === true) {
                    echo '<script>window.location.href="ingredientes_lista.php?report=ingrediente-editado"</script>';
                }
            }else{
                $sql = "UPDATE ingredientes SET nome='$update_nome' WHERE id_ingrediente=$update_id ";
                if ($conn->query($sql) === true) {
                    echo '<script>window.location.href="ingredientes_lista.php?report=ingrediente-editado"</script>';
                }
            }
