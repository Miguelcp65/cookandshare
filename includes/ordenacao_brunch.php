...
receitas.imagem, receitas.classificacao, categorias.nome AS 'catnome' FROM receitas  INNER JOIN categorias ON receitas.id_categoria = categorias.id_categoria
 WHERE receitas.privacidade = 'Público' AND receitas.id_categoria = 7 AND receitas.estado = 'Aprovado' ORDER BY classificacao DESC");
            ...
            if (isset($_GET["ordem"])){
                if ($_GET["ordem"] == "alfa") {
                   echo '<script> $(".selected").removeClass("selected"); document.getElementById("orderalfa").className = "selected";</script>';
                    ...
                    receitas.imagem, receitas.classificacao, categorias.nome AS 'catnome' FROM receitas INNER JOIN categorias ON receitas.id_categoria = categorias.id_categoria
                     WHERE receitas.privacidade = 'Público' AND receitas.id_categoria = 7 AND receitas.estado = 'Aprovado' ORDER BY nome ASC");
                ...
                receitas.imagem, receitas.classificacao, categorias.nome AS 'catnome'  FROM receitas INNER JOIN categorias ON receitas.id_categoria = categorias.id_categoria
                 WHERE receitas.privacidade = 'Público' AND receitas.id_categoria = 7 AND receitas.estado = 'Aprovado' ORDER BY criado DESC");
              echo("<script>history.replaceState({},'','$url');</script>");
